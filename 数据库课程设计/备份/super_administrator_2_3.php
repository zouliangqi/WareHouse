...
<?php
      $select_report =   "select * from {$db_name}.report";
      $check_query = mysqli_query($link,$select_report) or die(mysqli_error($link));
      echo "<p>";
      echo"<table  border=1>";
      echo"<tr><td>操作员工号</td><td>类型</td><td>时间</td><td>物品名</td><td>数量</td></tr>";
      while($row = mysqli_fetch_assoc($check_query)){
      echo"<tr >";
      echo"<td>".$row['rano']."</td>";
      echo"<td>".$row['rtype']."</td>";
      echo"<td>".$row['rtime']."</td>";
      echo"<td>".$row['rgname']."</td>";
      echo"<td>".$row['rgnumber']."</td>";
      echo"</tr>";
  }
      echo"</table>";
      mysqli_close($link);
